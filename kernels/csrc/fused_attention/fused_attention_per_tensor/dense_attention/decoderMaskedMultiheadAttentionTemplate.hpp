...
#pragma unroll
    ...
    template <typename T, typename T_VEC, unsigned VECS_PER_CHUNK> // add by JXGuo: VECS_PER_CHUNK = THREADS_PER_KEY, 所以 idx_chunk 就是第几个key
    ...
    inline __device__ void masked_multihead_attention_kernel(
        ...
        #pragma unroll
                for (int i = 0; i < 2; i++)
                {
                    ...
                    // convert_from_float(&kv_scale_quant_orig[i], (ENABLE_8BITS_CACHE ? 1.0f : 1.0f));        // Placeholder for kv_scale_quant_orig
                }
        #pragma unroll
                for (int i = 0; i < 2; i++)
                {
                    convert_from_float(&kv_scale_orig_quant[i], (ENABLE_8BITS_CACHE ? params.kv_scale_orig_quant[i] : 1.0f));
                    // convert_from_float(&kv_scale_orig_quant[i], (ENABLE_8BITS_CACHE ? 1.0f : 1.0f));        // Placeholder for kv_scale_orig_quant
                }
        // NOTE (Shang): Newly added zero points
        if constexpr (ENABLE_ZEROS)
        {
            #pragma unroll
            for (int i = 0; i < 2; i++)
            {
                // convert_from_float(&kv_zero[i], (ENABLE_8BITS_CACHE ? params.kv_zero[i] : 0.0f));
                convert_from_float(&kv_zero[i], (ENABLE_8BITS_CACHE ? 0.0f : 0.0f));        // Placeholder for kv_zero
            }
        }
            ...
            if (handle_kv && hi == (hi_kv * qhead_per_kv) && (IS_Dh_MAX || is_valid_qk_vec))
            {
                ...
                if constexpr (ENABLE_ZEROS)
                {
                    if constexpr (ENABLE_4BITS_CACHE)
                    {
                        ...
                        store_4bits_kv_cache_vec(reinterpret_cast<Tcache *>(k_cache), k, inBlockIdx, kv_scale_orig_quant[0], kv_zero[0]);
                    }
                    else if constexpr (ENABLE_8BITS_CACHE)
                    {
                        ...
                        store_8bits_kv_cache_vec(reinterpret_cast<Tcache *>(k_cache), k, inBlockIdx, kv_scale_orig_quant[0], kv_zero[0]);
                    }
                    else
                    {
                ...
                else
                {
                    if constexpr (ENABLE_4BITS_CACHE)
                    {
                        ...
                        store_4bits_kv_cache_vec(reinterpret_cast<Tcache *>(k_cache), k, inBlockIdx, kv_scale_orig_quant[0]);
                    }
                    else if constexpr (ENABLE_8BITS_CACHE)
                    {
                        // store_8bits_kv_cache_vec(reinterpret_cast<Tcache *>(k_cache), k, inBlockIdx, k_scale_orig_quant);
                        store_8bits_kv_cache_vec(reinterpret_cast<Tcache *>(k_cache), k, inBlockIdx, kv_scale_orig_quant[0]);
                    }
                    else
                    {
                        *reinterpret_cast<Qk_vec_m *>(&k_cache[inBlockIdx]) = vec_conversion<Qk_vec_m, Qk_vec_k>(k);
...
if constexpr (ENABLE_4BITS_CACHE && ENABLE_ZEROS)
    ...
    #pragma unroll
        ...
        const int global_valid_context_length = IS_RETRIEVAL_HEAD ? context_length: min(kvCacheBuffer.sinkTokenLen + kvCacheBuffer.localTokenLen - 1, context_length);
        ...
        const int sink_local_gap = context_length - global_valid_context_length;
        ...
        // if ((threadIdx.x == 0 || threadIdx.x == 20) && threadIdx.y == 0 && threadIdx.z == 0){
        //     printf("tidx: %d, k_idx.x: %d, k_idx.y: %d, THREADS_PER_KEY: %d, UNROLLED_K_PER_ITER: %d, context_ti_end: %d, K_PER_ITER: %d, K_LOOP_UNROLL: %d\n", tidx, k_idx.x, k_idx.y, THREADS_PER_KEY, UNROLLED_K_PER_ITER, context_ti_end, K_PER_ITER, K_LOOP_UNROLL);
        // }
        if constexpr (ENABLE_ZEROS)
        {
...
#pragma unroll
                for (int k_loop = 0; k_loop < K_LOOP_UNROLL; ++k_loop)
                {
                    ...
                    const int _logic_time_now = logic_time_base + k_loop * K_PER_ITER;
...
#pragma unroll
                    for (int k_vec_i = 0; k_vec_i < K_VECS_PER_THREAD; ++k_vec_i)
                    {
                        ...
                        if constexpr (ENABLE_4BITS_CACHE)
                        {
                            ...
                            convert_from_4bit_kv_cache<K_vec_m, K_vec_k, Tcache, T_scale>(
                                &k_vec[k_vec_i], k_vec_cache[k_loop][k_vec_i], kv_scale_quant_orig[0], kv_zero[0]);
                        }
                        else if constexpr (ENABLE_8BITS_CACHE)
                        {
...
#pragma unroll
                    for (int k_vec_i = 0; k_vec_i < K_VECS_PER_THREAD; ++k_vec_i)
                    {
                        ...
                        if constexpr (ENABLE_4BITS_CACHE)
                        {
                            ...
                            convert_from_4bit_kv_cache<K_vec_m, K_vec_k, Tcache, T_scale>(
                                &k_vec[k_vec_i], k_vec_cache[k_loop][k_vec_i], kv_scale_quant_orig[0]);
                        }
                        else if constexpr (ENABLE_8BITS_CACHE)
                        {
                            // convert_from_8bit_kv_cache<K_vec_m, K_vec_k, Tcache, T_scale>(
...
#pragma unroll
        ...
        for (int ti = tidx; ti < physic_logit_loop_end; ti += THREADS_PER_BLOCK)
        {
            ...
            const int _logic_time_now = ti + c_tile_times_timesteps_per_block;
            const int logic_time_now = _logic_time_now >= kvCacheBuffer.sinkTokenLen ? _logic_time_now + sink_local_gap : _logic_time_now;
...
#pragma unroll
                    for (int v_loop = 0; v_loop < V_LOOP_UNROLL; v_loop++)
                    {
                        ...
                        const int _logic_time_now = logic_time_base + v_loop * V_PER_ITER;
                        // const int logic_time_now = _logic_time_now >= kvCacheBuffer.sinkTokenLen ? _logic_time_now + sink_local_gap : _logic_time_now;
                        const int logic_time_now = min(_logic_time_now >= kvCacheBuffer.sinkTokenLen ? _logic_time_now + sink_local_gap : _logic_time_now, context_length - 1);
...
#pragma unroll
                    for (int v_loop = 0; v_loop < V_LOOP_UNROLL; v_loop++)
                    {
                        ...
                        if constexpr (ENABLE_4BITS_CACHE)
                        {
                            ...
                            convert_from_4bit_kv_cache<V_vec_m, V_vec_k, Tcache, T_scale>(
                                &v_vec, v_vec_cache[v_loop], kv_scale_quant_orig[1], kv_zero[1]);
                        }
                        else if constexpr (ENABLE_8BITS_CACHE)
                        {
...
#pragma unroll
                    for (int v_loop = 0; v_loop < V_LOOP_UNROLL; v_loop++)
                    {
                        ...
                        if constexpr (ENABLE_4BITS_CACHE)
                        {
                            ...
                            convert_from_4bit_kv_cache<V_vec_m, V_vec_k, Tcache, T_scale>(
                                &v_vec, v_vec_cache[v_loop], kv_scale_quant_orig[1]);
                        }
                        else if constexpr (ENABLE_8BITS_CACHE)
                        {
                            // convert_from_8bit_kv_cache<V_vec_m, V_vec_k, Tcache, T_scale>(
            ...
            if (hi == (hi_kv * qhead_per_kv))
            {
                if constexpr (ENABLE_ZEROS)
                {
                    if constexpr (ENABLE_4BITS_CACHE)
                    {
                        ...
                        store_4bits_kv_cache_vec(v_cache_base, v, inBlockIdx, kv_scale_orig_quant[1], kv_zero[1]);
                    }
                    else if constexpr (ENABLE_8BITS_CACHE)
                    {
                        // store_8bits_kv_cache_vec(v_cache_base, v, inBlockIdx, v_scale_orig_quant, v_zeros);
                        store_8bits_kv_cache_vec(v_cache_base, v, inBlockIdx, kv_scale_orig_quant[1], kv_zero[1]);
                    }
                    else
                    {
                ...
                else
                {
                    if constexpr (ENABLE_4BITS_CACHE)
                    {
                        ...
                        store_4bits_kv_cache_vec(v_cache_base, v, inBlockIdx, kv_scale_orig_quant[1]);
                    }
                    else if constexpr (ENABLE_8BITS_CACHE)
                    {
                        // store_8bits_kv_cache_vec(v_cache_base, v, inBlockIdx, v_scale_orig_quant);
                        store_8bits_kv_cache_vec(v_cache_base, v, inBlockIdx, kv_scale_orig_quant[1]);
                    }
                    else
                    {
                        *reinterpret_cast<V_vec_k *>(&v_cache_base[inBlockIdx]) = v;
...
#pragma unroll
    ...
    __global__ void masked_multihead_attention_compute(
        Multihead_attention_params<T> params, RetrievalKVCacheBuffer retrieval_kv_buffer, StreamingKVCacheBuffer streaming_kv_buffer){
            ...
            if (is_retrieval_head){
                masked_multihead_attention_kernel<T, Tcache, RetrievalKVCacheBuffer, Dh, THREADS_PER_BLOCK, true, DO_MULTI_BLOCK, INT4KV, KV_WITH_ZEROS, SMEM_PRELOAD, THREADS_PER_KEY, THREADS_PER_VALUE, V_LOOP_UNROLL>(params, retrieval_kv_buffer, head_rank);
            }else{
                masked_multihead_attention_kernel<T, Tcache, StreamingKVCacheBuffer, Dh, THREADS_PER_BLOCK, false, false /*DO_MULTI_BLOCK*/, INT4KV, KV_WITH_ZEROS, SMEM_PRELOAD, THREADS_PER_KEY, THREADS_PER_VALUE, V_LOOP_UNROLL>(params, streaming_kv_buffer, head_rank);
            }
        }
    ...
    inline size_t smem_size_in_bytes(const Multihead_attention_params<T> &params, int threads_per_block)
    {
        ...
        const int max_timesteps = max(min((DO_MULTI_BLOCK ? params.timesteps_per_block : params.timestep), params.memory_max_len), params.streaming_sink_token_num + params.streaming_local_token_num);
