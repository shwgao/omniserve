    ...
    int threads_per_block, int tlength, bool do_multi_block)
{
    ...
    // added by JXGuo: For example, if a block has 256 threads and each value requires 8 threads, then seq_len_tile=32, allowing a block to process 32 sequence segments simultaneously.
    ...
    const KernelParamsType& params, const RetrievalKVCacheBuffer& retrieval_kv_buffer, const StreamingKVCacheBuffer& streaming_kv_buffer, const cudaStream_t& stream)
{
    ...
    if (int4_kv_cache)
    {
        if (tlength < params.multiblock_switch)
        {
