    ...
    int threads_per_block, int tlength, bool do_multi_block)
{
    ...
    // JXGuo: if a block has 256 threads and each value requires 8 threads, then seq_len_tile = 32, allowing one block to process 32 sequence segments simultaneously.
